...
- Apesar de que nos vídeos as demonstrações utilizam versões mais antigas do Java e do JUnit, **os exercícios devem ser realizados em Java 21 com JUnit 5**. Os métodos de teste são muito similares entre as versões; caso encontre alguma diferença, consulte a [documentação oficial do JUnit 5](https://docs.junit.org/5.14.4/overview.html).
...
- Devem existir **obrigatoriamente** na solução entregue arquivos `.java` em `src/main/java` com os seguintes nomes de classe:
  - Negociacao
  - Clube
  - Goleiro
  - Atacante
...
- A pipeline de CI valida a presença desses arquivos conforme listado em `expected_classes.txt` na raiz do repositório.
